...
/**
 * DIC security checks: transport security (HTTPS, no mixed content), session
...
test.describe('DIC security', () => {
  ...
  test.describe('Unauthenticated', () => {
    test('DIC deep-link without a session redirects to the login screen', async ({ page }) => {
    ...
    test('clearing the session token locks the DIC dashboard on reload', async ({ page }) => {
      await login(page);
      // Drop the token then attempt a direct DIC load — the guard must block it.
