...
/**
 * DIC performance budget: time-to-interactive for the WebGL dashboard, a live
 * FPS / polygon HUD, a bounded console-error count and a reasonable DOM size.
 * Budgets are deliberately generous because DIC renders a heavy 3D scene.
 */
test.describe('DIC performance', () => {
  test.describe.configure({ timeout: 150_000 });

  test('becomes interactive within the load budget', async ({ page }) => {
    await login(page);
    const start = Date.now();
    await page.locator('.facility-card[data-href="DIC/index.html"]').click();
    const dic = new DicPage(page);
    await dic.expectLoaded();
    const elapsed = Date.now() - start;
    expect(elapsed, `DIC interactive in ${elapsed}ms`).toBeLessThan(45_000);
  });
  ...
  test('loads without a flood of console errors', async ({ page }) => {
    ...
    const dic = new DicPage(page);
    await dic.expectLoaded();
    // Tolerate a few benign 3rd-party/telemetry errors, but flag a flood.
