...
/**
 * DIC UI / UX checks: typography (Plus Jakarta Sans), the dark theme palette,
 * toolbar alignment and absence of layout overflow. These assert the brand
 * styling and layout integrity highlighted across the reference screenshots.
 */
test.describe('DIC UI / visual', () => {
  ...
  test('renders the dashboard title text', async ({ page }) => {
    await expect(page.locator('.top-title')).toHaveText(/ALEC DIC YARD/i);
  });
  ...
  test('aligns all toolbar buttons on a single horizontal row', async () => {
    ...
    const tops = await dic.bottomBar.locator('button').evaluateAll((btns) =>
    ...
    // Every visible toolbar button shares the same top offset (±2px).
  ...
  test('keeps the toolbar within the viewport', async ({ page }) => {
    const box = await dic.bottomBar.boundingBox();
  ...
  test('renders the top bar, zone navigator and side panels without clipping', async ({ page }) => {
    for (const sel of ['#top-bar', '#zone-nav', '#panel-stat-cards', '#panel-ai-score']) {
